...
- [ ] **Verify OPENAI_API_KEY is in `.env`** — AI features (Smart Add and recipe extraction) will silently fail or error without it.
...
- [ ] **Finish hardening recipe URL extraction** — the live grocery extraction route validates URL schemes, blocks internal addresses and unsafe redirects, and sets client timeouts, but response bodies are still unbounded. Cap the downloaded response size before treating this as complete.
...
- [ ] **Add reliable tests** for critical application flows.
...
- [ ] **Finish recipe-fetch hardening** — user-supplied URLs are fetched by the live grocery extraction route. `internal/ingredient/extractor.go` allows only HTTP(S), blocks loopback/private/link-local targets in the dialer, restricts redirects, and sets client/dial timeouts. The response body is still unbounded; cap it with `io.LimitReader` (and reject oversized responses) to prevent memory/processing abuse.
...
- [ ] **AI endpoints: unbounded input + no rate limit = cost abuse** — `smart-add` text and recipe-extraction links have no length cap, there is no request body limit, and AI routes have no rate limiting (login is the only limited route). Add a global body-size limit (`http.MaxBytesHandler` or middleware), explicit input caps, and a per-user/household limiter on AI routes.
- [ ] **No deadlines on outbound calls** — OpenAI calls (`internal/ai/client.go:24`) and recipe page fetches run without request-level deadlines; callers pass `gin.Context`, whose `Done()/Deadline()` are no-ops by default. Pass `c.Request.Context()` wrapped in `context.WithTimeout` (or set `engine.ContextWithFallback = true`).
- [ ] **`FromRecipeURL` ignores the AI error** — after the page fetch succeeds, `internal/ingredient/extractor.go` assigns the AI extraction error but never checks it, so an API failure returns an empty list as if extraction succeeded. Return the error.
...
- [ ] **No indexes on FK/tenant columns** — `init.sql` has zero `CREATE INDEX`. Add indexes on `grocery_lists.household_id`, `groceries.grocery_list_id`, `groceries.product_id`, `sessions.user_id`, `sessions.expires_at`, `invites.household_id`, and `todos.household_id`.
...
- [ ] **Internal error text leaked to clients** — `c.String(500, err.Error())` at `todo/handler.go:97,131`, `grocery/handler.go:39`, and `notification/handler.go:27,47`. Log server-side, return generic messages.
...
- [ ] **Stop logging user/AI content to stdout** — `ai/client.go:42` prints full AI responses; recipe extraction and `todo/service.go:52` also contain debug output. Remove or use a leveled logger. Add Docker `logging` max-size to both compose services (unbounded json-file logs on an SD card). (PC)
- [ ] **Add security headers** — no CSP/HSTS/`X-Content-Type-Options`/`Referrer-Policy` anywhere. Add a small security-headers middleware (or set at the Cloudflare layer). ~~Vendor htmx/fonts~~ (done 2026-07-09 as part of offline mode: all CDN deps now in `web/static/vendor/`).
...
- [ ] Dead code: `extractor.go` contains an unused `findJSON` helper; `product.Service.AIService` is also unused. Delete or branch.

### Before re-enabling disabled routes (todos / home)
...
- [x] Missing templates: `home/handler.go:118` renders `"ai_extraction.html"`, which does not exist; `pages/index.html:8` references `{{template "header" .}}` but the define is `"layouts/header"`. Re-enabling these routes 500s immediately.
- [ ] Home pages never set `CSRFToken` in template data — HTMX mutations there will 403 once CSRF is enabled. Consider injecting the token via middleware instead of per-handler.
